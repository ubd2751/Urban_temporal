...
#try_seed <-
  ...
  mutate(
     seed_mass = case_when(
      str_detect(OrigUnitStr, pattern = "mg") ~ OrigValueStr*1000,
      str_detect(OrigUnitStr, pattern = "1000") ~ OrigValueStr/1000,
      str_detect(OrigUnitStr, pattern = "kg") ~ OrigValueStr*1000,
      OrigUnitStr == "g/100 seeds"   ~ OrigValueStr/100,
      OrigUnitStr == "mg/1000" ~ OrigValueStr,
      OrigUnitStr == "g / 1000 seeds" ~ OrigValueStr/1000,
      OrigUnitStr == "micro gx10"   ~ OrigValueStr
      )
  )
...
try_life <-
  ...
  dplyr::group_by(AccSpeciesName) %>% 
  dplyr::summarise(across(annual:tree, sum)) %>% 
  pivot_longer(-AccSpeciesName, names_to = "life") %>% 
  dplyr::group_by(AccSpeciesName) %>% 

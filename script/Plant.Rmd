...
```{r}
df_growth <- 
  dplyr::bind_rows(try_life, aust_life) %>% 
  dplyr::group_by(sciname) %>% 
  dplyr::summarise(across(where(is.numeric), max)) 
```
...
pic_disper <- 
  pic_data %>% 
  dplyr::select(species, dispersal) %>% 
  ...
  dplyr::left_join(select(plant_sci, species, sciname), by = "species") %>% 
  dplyr::left_join(select(plant_sci, speciesB, sciname), 
            by = c("species" = "speciesB")) %>% 
  dplyr::mutate(name = if_else(is.na(sciname.x), sciname.y, sciname.x)) %>% 
  dplyr::distinct(species, .keep_all = TRUE) %>% 
  dplyr::select(name, dispersal) %>% 
...
```{r}
df_disper <- 
  bind_rows(try_disper, df_pic, aust_disper, leda_disper) %>% 
  dplyr::group_by(sciname) %>% 
  dplyr::summarise(across(where(is.numeric), max))
```
...
plant %>% 
  ...
  dplyr::left_join(df_trait, by = "species") %>% {
    ...
    dplyr::filter(., lost == 1) %>% 
      dplyr::group_by(site, exotic) %>% 
      dplyr::mutate(across(wind:gravity, replace_na, 0)) %>% 
      dplyr::summarise(across(wind:gravity, sum)) %>% 

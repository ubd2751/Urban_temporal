...
ylist_detail <- 
  YList %>% 
  dplyr::select(和名, 別名, 学名, 生態)　%>% 
  dplyr::rename(species = 和名,  other_name = 別名, sciname = 学名, 
         exotic = 生態) %>% 
  dplyr::filter(species != "") %>% 
  dplyr::distinct(species, .keep_all = TRUE) %>% 
  dplyr::mutate(exotic = recode(
    exotic,
    "帰" = "immigration",
    "栽" = "ornament",
    "外" = "exotic",
...
Boxplot for comparison of species richnes
...
box_sr_plant <- 
  ggplot(data = sr_plant, aes(x = time, y = sr)) +
    ...
    geom_signif(
      data = data.frame(
        exotic = factor(
          c("All species","Native species","Exotic species"),
          levels = c("All species","Native species","Exotic species"))),
      aes(y_position = c(850, 800, 200), 
          xmin = c(1,1,1), xmax = c(2, 2, 2),
          annotations = c("p = 0.302", "p = 0.148", "p = 0.711")),
          manual = TRUE, size = 0.2, textsize = 2) +
...
df_plant_lost %>% 
    dplyr::mutate(across(where(is.character), as.factor)) %>% 
    dplyr::filter(exotic == "Native" & !is.na(growth) & !is.na(disper))  ->> test
    
glm(lost ~ area + green_rate + growth + disper, 
        family = "binomial", data =test) %>%  summary()
  rsq.partial(adj = TRUE) %>% 
  pluck(3)
```
...
box_plant_lost_growth <- 
  df_plant_lost %>% 
    ...
    # plot
    ggplot() +
    geom_boxplot(aes(x = growth, y = lost, fill = growth)) + 
    facet_grid(~exotic) +
    geom_text(
      data = cha_glm_growth, 
...
ggsave(
  box_plant_lost_growth,
  file = "output/box_plant_growth.png",
...
box_plant_lost_disper <- 
  df_plant_lost %>% 
    ...
    # plot
    ggplot() +
    geom_boxplot(aes(x = disper, y = lost, fill = disper)) + 
    facet_grid(~exotic) +
    geom_text(
      data = cha_glm_lost_disper, 
      aes(x = growth, y = 1.05, label = letters),
      position = position_dodge(width = 1), size = 3) +
    labs(
      x = "Dispersal type",
...
ggsave(
  box_plant_disper,
  file = "output/box_plant_disper.png",
  width = 100, height = 60, units = "mm", dpi = 500)
```
